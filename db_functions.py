...
def verify_initiator(cursor, auth_key):
  ...
  for _ in cursor.fetchall():
    return 1   # initiator verified
  return 0   # not there

def update_login(cursor,username,auth_key,p):
  
  query = f"""
  UPDATE Login
  SET Auth_key = '{auth_key}', PORT = {p}
  WHERE Username = '{username}';
  """
  cursor.execute(query)
...
while True:
  ...
  if(ch==1):
    ...
    update_login(cursor=cursor, username=username, auth_key=auth_key, p=p)
    # conn.commit()
  elif(ch==3):
    username=input("Enter username: ")
